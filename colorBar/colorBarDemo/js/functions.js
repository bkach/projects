  ...
  float sinus(float rotation, float zoom, float power,float intensity){
      ...
      float sine = 
        pow(
          cos(
            (P.x * sin(-radians) + 
             P.y * cos(-radians)) / zoom + (turbulence(vec4(P/zoom,1.0))) * intensity),
        power);
      return sine;
  }
...
// Add multiple sections
  ...
  cb1.sections.push(
      new Section(
          new BreakPoint(new THREE.Vector3(0.0,0.0,1.0), 9.0 , 0.750),
          `
          mixValue = snoise(vec4(vPosition / 30.0,1.0));
          `,
...
//cb1.sections.push(
    //new Section(
      ...
      //mixValue = line1;
      //float line2 = line(0.0,1.0,1.0,0.0);
...
`
  vec4 finalColor(){
    ...
    // Noise
    //float noise = bound(harmonicSeries(vec4(vPosition/100.0,1.0)));
    //return colorBar( noise ).color;
    // Turb
    //return colorBar(bound(turbulence(vec4(vPosition/40.0,1.0)))).color;
  }
  float finalDisplacement(){
    ...
    // Noise
    //return colorBar( bound(harmonicSeries(vec4(vPosition/100.0,1.0))) ).displacement;
    // Turb
